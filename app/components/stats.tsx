...
export const Stats = asyncComponent(async () => {
  const [reads, writes] = await redis
    ...
    .exec<[number, number]>();
  const stars = await fetch("https://api.github.com/repos/vishal11u/envshare-dev-v")
    .then((res) => res.json())
    ...
    <section className="container mx-auto">
      <ul className="grid grid-cols-1 gap-4 sm:grid-cols-3 ">
        {stats.map(({ label, value }) => (
          <li
            key={label}
            className="flex border flex-col items-center justify-between gap-2 px-4 py-4 overflow-hidden rounded-sm sm:flex-col"
          >
